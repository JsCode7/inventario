...
export default function LogsInventoryPage() {
    ...
    fetch("/api/logs")
      ...
      .then((data) => {
        const filteredLogs = data.data.filter((log) =>
          log.action.includes("/api/inventory")
        );
  ...
  const exportToPDF = () => {
    const doc = new jsPDF();
    doc.text("Logs de Inventario", 20, 10);
    let rowIndex = 20;

    filteredLogs.forEach((log) => {
      doc.text(
        `${new Date(log.createdAt).toLocaleString()} - ${log.method} - ${
          log.action
        }`,
        20,
        rowIndex
  ...
  return (
    <div className="min-h-screen bg-gray-900 text-white p-8">
      <h1 className="text-3xl font-bold mb-6">Logs de inventario</h1>
      ...
      <button
        onClick={exportToPDF}
        className="bg-green-500 p-3 mb-6 rounded-lg flex items-center space-x-2"
      >
      ...
      {filteredLogs.length === 0 ? (
        <p className="text-gray-400">No logs found for /api/inventory.</p>
      ) : (
        filteredLogs.map((log) => (
          <div
            className="m-2 p-4 bg-gray-800 rounded-lg shadow-lg"
            key={log._id}
          >
            ...
            <div className="mb-2 flex justify-between items-center">
              ...
              >
                <FontAwesomeIcon
                  icon={expandedLogs[log._id] ? faChevronUp : faChevronDown}
                />
              </button>
            </div>
